...
function doPost(event) {
  try {
    ...
    // Inicializar pestañas necesarias en la hoja
...
/**
 * 1. LOGIN: Valida credenciales contra la pestaña Usuarios y genera token de sesión
 */
function handleLogin(payload) {
  ...
  // Verificar hash con el salt almacenado
  const computedHash = computeHash(password, user.salt);
  if (computedHash !== user.passwordHash) {
    return { ok: false, message: "Correo o contraseña incorrectos." };
  }

  // Generar token de sesión único y almacenarlo en CacheService
  ...
  // Actualizar último acceso en la hoja
...
/**
 * 2. VERIFY_SESSION: Validación obligatoria del token del lado del servidor
 */
...
/**
 * 3. UPDATE_STATUS: Permite al rol Kiosco cambiar el estado de un turno
 */
function handleUpdateStatus(payload) {
  ...
  for (let i = data.length - 1; i >= 1; i--) {
    ...
    if ((idTurno && rowId === idTurno) || (dni && rowDni === dni)) {
      sheet.getRange(i + 1, 10).setValue(nuevoEstado); // Columna J (ESTADO)
      rowUpdated = true;
      break;
    }
  }
...
/**
 * 4. CREATE: Registra un nuevo turno público en la pestaña Turnos
 */
function handleCreateAppointment(payload) {
  ...
  // Generación automática de REGISTRO e ID TURNO
  const now = new Date();
...
/**
 * 5. LOOKUP: Búsqueda de turno por DNI (desde la fila más reciente a la más antigua)
 */
...
function ensureSheetsInitialized() {
  ...
  if (!usersSheet) {
    ...
    // Si solo están las cabeceras pero no hay usuarios, sembrar usuario inicial
    seedInitialUser(usersSheet);
  }
}

function seedInitialUser(usersSheet) {
  // Usuario inicial habilitado requerido
